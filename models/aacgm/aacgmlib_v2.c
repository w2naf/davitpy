...
;
; 20141031 NAF v1.1  Added MLT Calculations from old RST distribution.
;                    MLT Function names/API match AACGM_v2 conventions.
;                    Fixed bugs compared to RST code:
;                       * Decimal day error in AstAlg.c
;                       * Julian day offset in AACGM_v2_ConvertMLT()
;                       * Height is no longer hard-coded in MLT conversions.
;
; Functions:
;
; AACGM_v2_Rylm
...
;
; PURPOSE:
;       Calculate Magnetic Local Time as a function of
;           * apparent solar time [UT+Equation of Time, in seconds]
;           * solar declination angle
;           * altitude
;           * magnetic longitude
;
; CALLING SEQUENCE:
;       mlt = astmlt1(at, dec, mlon, height, mslong);
;     
;     Input Arguments:  
;       at            - apparent solar time [seconds] (UT+Eq of Time)
;       solar_dec     - solar declination angle [deg]
;       mlon          - Magnetic Longitude [deg]
;       height        - altitude [km]
;
;     Output Arguments:  
;       mslon         - Mean Solar Longitude [deg] (Location of the Sun)
;
;     Return Value:
...
double astmlt1(int t0,double solar_dec,double mlon,double height,double *mslon) {
    double ret_val;
    double mslat1,mslat2,slon1,slon2;
    double r;
    ...
    if (fabs(mslon2-mslon1)>10) told=1e12;
        ...
        if ((t0>=told) && (t0<(told+600))) {
            ...
            */
            ...
            // Now using top-level functions // NAF 20141101
            AACGM_v2_Convert(solar_dec,slon1,height,&mslat1,&mslon1,&r,0);
            AACGM_v2_Convert(solar_dec,slon2,height,&mslat2,&mslon2,&r,0);

            //convert_geo_coord(solar_dec,slon1,height,&mslat1,&mslon1,0,4);
            //convert_geo_coord(solar_dec,slon2,height,&mslat2,&mslon2,0,4);
...
;
; PURPOSE:
;       Calculate Magnetic Local Time for a given magnetic longtitude and
;       altitude.  Use AACGM_v2_SetDateTime() to set the date and time used by
;       this function.
;
; CALLING SEQUENCE:
;       err = AACGM_v2_ConvertMLT(mlon, height, mlt, mslong);
;
;     Input Arguments:  
;       mlon          - Magnetic Longitude [deg]
;       height        - altitude [km]
...
;
;     Return Value:
;       err           - Error Code (0 if all is right with the world)
;     
;+-----------------------------------------------------------------------------
*/

int AACGM_v2_ConvertMLT(double mlon, double height, double *mlt, double *mslong) {
    double jd,dd,eqt,dec,ut,at;

    /* no date/time set so use current time */
    if (aacgm_date.year < 0) {
            msg_notime();
            return -128;
    }

    dd  = AstAlg_dday(aacgm_date.day,aacgm_date.hour,aacgm_date.minute,aacgm_date.second);    // Decimal day (I am not sure why we subtract 1 from the day here...)
    jd  = AstAlg_jde(aacgm_date.year,aacgm_date.month,dd);                  // Julian day
    eqt = AstAlg_equation_of_time(jd);                                      // Equation of time [minutes]
    dec = AstAlg_solar_declination(jd);                                     // Solar declination in degrees.
    ut  = aacgm_date.hour*3600 + aacgm_date.minute*60 + aacgm_date.second;  // UT time in seconds
    at  = ut + eqt*60.0;                                                    // Apparent solar time (in seconds) in UT timezone.

    *mlt = astmlt1(at, dec, mlon, height, mslong);

    #if DEBUG > 0
    printf("\n====== BEGIN DEBUG FOR AACGM_v2_ConvertMLT() ======\n");
    printf("YMDHMS: %d %d %d %d %d %d %lf\n",aacgm_date.year,aacgm_date.month,aacgm_date.day,aacgm_date.hour,aacgm_date.minute,aacgm_date.second,mlon);
    printf("YMdd: %04d %02d %lf\n",aacgm_date.year,aacgm_date.month,dd);
    printf("Julian day: %f\n",jd);
    printf("EQT [min]: %lf\n",eqt);
    printf("Solar Dec [deg]: %lf\n",dec);
    printf("UT [sec]: %lf\n",ut);
    printf("Apparent Solar Time [sec]: %lf\n",at);
    printf("mlon [deg]: %lf\n",mlon);
    printf("height [deg]: %lf\n",height);
    printf("\n");
    printf("mlt: %lf\n",*mlt);
    printf("====== END DEBUG FOR AACGM_v2_ConvertMLT() ======\n");
    #endif
